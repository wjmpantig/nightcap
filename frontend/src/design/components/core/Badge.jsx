...
const TONES = {
  ...
  awake: { color: 'var(--state-awake)', bg: 'var(--amber-a12)', border: 'color-mix(in oklch,var(--state-awake) 28%,transparent)' },
  watched: { color: 'var(--state-watched)', bg: 'var(--moonlight-a08)', border: 'var(--moonlight-a32)' },
  closed: { color: 'var(--state-closed)', bg: 'var(--ember-a12)', border: 'color-mix(in oklch,var(--state-closed) 28%,transparent)' },
...
export function Badge({ tone = 'neutral', dot = false, icon, shape = 'pill', children, style, ...rest }) {
  ...
  return (
    <span className={'nc-badge' + (shape === 'square' ? ' nc-badge--square' : '')}
      style={{ color: t.color, background: t.bg, borderColor: t.border, ...style }} {...rest}>
