...
export function Mark({ size = 24, variant = 'solid', state = 'active', title, style, ...rest }: MarkProps) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none"
      role={title ? 'img' : 'presentation'} aria-label={title} style={style} {...rest}>
      {title && <title>{title}</title>}
      <clipPath id={id}><rect x="0" y="0" width="100" height={clipH} /></clipPath>
      {variant === 'outline'
        ? <circle cx="50" cy={cy} r="33.5" fill="none" stroke={discFill} strokeWidth="9" clipPath={'url(#' + id + ')'} />
