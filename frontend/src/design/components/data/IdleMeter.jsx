...
export function IdleMeter({ idleSecs, timeoutMinutes, label = 'Idle', style, ...rest }) {
  ...
  return (
    <div {...rest} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', minWidth: 180, ...style }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 'var(--space-4)' }}>
        ...
        <span style={{ font: 'var(--type-mono)', color: near ? 'var(--state-awake)' : 'var(--text-primary)' }}>{mmss(idleSecs)}</span>
        <span style={{ marginLeft: 'auto', font: 'var(--type-mono-sm)', color: 'var(--text-muted)' }}>/ {timeoutMinutes}m</span>
      </div>
      <div style={{ height: 3, borderRadius: 'var(--radius-pill)', background: 'var(--night-700)', overflow: 'hidden' }}>
        <div style={{
          width: (frac * 100).toFixed(2) + '%', height: '100%',
