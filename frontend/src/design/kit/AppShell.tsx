...
import { HISTORY, REQUESTS, WATCHLIST } from "./mock"
...
export function AppShell() {
  ...
  useEffect(() => {
    if (!pending) return
    const t = setInterval(
      () => setPending((p) => (p && p.remaining > 0 ? { ...p, remaining: p.remaining - 1 } : p)),
      1000,
    )
    return () => clearInterval(t)
  }, [pending])
  ...
  const armed = entries.filter(
    (e) => !e.snoozedUntil || new Date(e.snoozedUntil) < new Date(),
  ).length

  const watch = (exe: string) =>
    setEntries((es) =>
      es.some((e) => e.exe === exe) ? es : [...es, { exe, timeoutMinutes: 0, snoozedUntil: "" }],
    )
  const setTimeoutFor = (exe: string, m: number) =>
    setEntries((es) => es.map((e) => (e.exe === exe ? { ...e, timeoutMinutes: m } : e)))
  const snooze = (exe: string, mins: number) =>
    setEntries((es) =>
      es.map((e) =>
        e.exe === exe
          ? {
              ...
              snoozedUntil:
                mins === 0
                  ? ""
                  : new Date(Date.now() + (mins < 0 ? 3.15e10 : mins * 60e3)).toISOString(),
            }
          : e,
      ),
    )
  ...
  const fire = () =>
    setPending({ apps: ["vlc.exe"], remaining: cfg.warningSeconds, total: cfg.warningSeconds })
  const counts: Partial<Record<ViewId, number>> = {
    awake: requests.filter((r) => r.exe !== "").length,
  ...
  return (
    <div className="win">
      ...
      >
        ...
        >
          <Button size="sm" variant="ghost" icon="triangle-alert" onClick={fire}>
            Simulate kill
          </Button>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <main className="scroll">
          ...
          {view === "watchlist" && (
            <WatchlistView
              entries={entries}
              defaultTimeout={cfg.defaultTimeoutMinutes}
              onSetTimeout={setTimeoutFor}
              onSnooze={snooze}
              onRemove={remove}
              onAdd={watch}
            />
          ...
          {view === "settings" && (
            <SettingsView cfg={cfg} onChange={(patch) => setCfg((c) => ({ ...c, ...patch }))} />
          )}
        </main>
      </div>
      ...
      {pending && (
        <WarningOverlay
          ...
          idleSecs={idleSecs}
          onSnooze={(mins) => {
            for (const a of pending.apps) snooze(a, mins)
            setPending(null)
          }}
